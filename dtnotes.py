...
def all_same_class(instances, class_col_index):
    #get the class_col using one of the myutils get col
    class_col = myutils.get_col_byindex(instances, class_col_index)
    #grab first class label from that col
    prev_label = class_col[0]
     #loop through each row and compare class label with first
    for label in class_col:
        if prev_label != label:
...
#dot -Tpdf -o interview_tree.pdf interview_tree.dot
